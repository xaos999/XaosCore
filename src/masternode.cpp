...
{
    if (nHeight >  1000000)  return    10000 * COIN;
    if (nHeight >  500000)  return     7000 * COIN;
    if (nHeight >  300000)  return     5000 * COIN;
    if (nHeight >  150000)  return     3000 * COIN;
    if (nHeight >       0)  return     2000 * COIN;
...
{
    if (nHeight >   1000000) return       25 * COIN;
    if (nHeight >    700000) return       21 * COIN;
    if (nHeight >    600000) return       18 * COIN;
    if (nHeight >    500000) return       15 * COIN;
    if (nHeight >    400000) return       12 * COIN;
    if (nHeight >    300000) return       9 * COIN;
    if (nHeight >    200000) return       6 * COIN;
    if (nHeight >    100000) return       4 * COIN;
    if (nHeight >         1) return        3 * COIN;
    if (nHeight >         0) return   170000 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 XCS tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 XCS tx -> 1 confirmation
